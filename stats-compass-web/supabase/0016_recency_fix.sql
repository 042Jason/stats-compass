...
--   표를 고르는 기준이 질의 벡터와의 거리뿐이라, 제목만 비슷하면
--   60년 전 자료도 같은 점수를 받았습니다.
--
-- 원래 목적:
-- 0011(성능)·0012(탈락 후보)의 내용을 모두 포함합니다.
--
-- 왜
--   화면이 통계표마다 <그 표의 주기>로 KOSIS 를 불러야 합니다.
--   연간 표에 월간을 요청하면 빈 결과가 옵니다.
--   props.prdSe / props.firstPeriod 를 검색 결과에 실어 보냅니다.
--   (12_buildOntology 를 다시 돌려야 props 에 값이 들어갑니다) (함수 전체를 다시 정의합니다)
--
-- 증상
...
tbl as (
  ...
  from ranked rk
  cross join lateral (
    -- 안쪽에서 <보정 거리(adj)>를 한 번 계산하고, 순번과 뽑기 둘 다 그걸로 합니다.
    --
    -- 0015 는 순번을 순수 거리로, 뽑기를 보정 거리로 매겼습니다. 기준이 둘로 갈리니
    -- 뽑힌 표의 순번이 1·2 가 아니라 5·9 가 되고, 바깥의 rn_in_survey <= 2 가
    -- 그것들을 다시 걸러내 통계표가 12개에서 4개로 줄었습니다.
    select z.label, z.props, z.tsim,
           row_number() over (order by z.adj) as rn_in_survey
    from (
      select e.label, e.props,
             (1 - (m.embedding <=> p_embedding))::numeric as tsim,
             -- 오래된 표에 벌점. 1960년 → 0.30(상한), 2021년 → 0.06, 2024년 → 0.02
             ...
             case
               when e.props->>'latestPeriod' ~ '^[0-9]{4}'
                 then least(
                        ...
                        greatest(
                          0,
                          extract(year from now())::int - left(e.props->>'latestPeriod', 4)::int
                        ) * 0.012
                      )
               else 0.15
             end as adj
...
seed_tbl as (
  ...
  from top_seeds s
  join public.ontology_entities e
    on e.id = s.entity_id and e.class_id = 'StatisticalTable'
  ...
  'tables', coalesce((
    select jsonb_agg(
             jsonb_build_object(
               ...
               'latestPeriod', props->>'latestPeriod',
               'firstPeriod', props->>'firstPeriod',
               'prdSe', props->>'prdSe',
               'survey', survey_label,
               'rank', rn,
               'directHit', rn = 0,
