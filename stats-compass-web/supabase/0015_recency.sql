-- ============================================================================
-- 0015_recency.sql — 오래된 통계표 밀어내기
--
-- 0014_table_periods.sql 다음에 실행합니다. 0011~0014 내용을 모두 포함합니다.
--
-- 증상
...
tbl as (
  ...
  cross join lateral (
    ...
    -- 유사도만 보면 1960년 통계표가 올라옵니다. 오래된 표에 벌점을 줍니다.
    --   벌점 = 경과연수 × 0.012, 최대 0.30
    --   1960년 → 0.30(상한)   2021년 → 0.06   2024년 → 0.02
    --   수록시점을 모르면 0.15(중간값). 코사인 거리가 대략 0.3~0.6 이므로
    --   이 정도면 최신 표를 앞세우되 관련성을 뒤집지는 않습니다.
    order by (m.embedding <=> p_embedding) +
             case
