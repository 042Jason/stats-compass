...
-- ---------------------------------------------------------------------------
-- 적용 확인 (마지막 SELECT 결과가 화면에 표시됩니다)
-- agency_손상 이 0 이고 search_all_동작 이 1 이상이면 정상입니다.
-- ---------------------------------------------------------------------------
select
  (select count(*) from public.statistics where agency like '국가데이터처_%') as agency_손상,
  (select count(*) from public.statistics where agency like '국가데이터처 %') as agency_복원됨,
  (select count(*) from public.search_all('인구', 40))                        as search_all_동작,
  (select count(*) from public.search_all('고용통계과', 10))                   as 기관명_검색;
